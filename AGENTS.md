...
Handy API is derived from the original Handy project. If work is explicitly
intended for upstream Handy instead of this repository, follow the contribution
rules in [cjpais/Handy](https://github.com/cjpais/Handy) separately. Do not use
the upstream project as the issue, discussion, or pull-request destination for
Handy API changes.
...
Handy API is a cross-platform desktop speech-to-text application built with Tauri 2.x (Rust backend + React/TypeScript frontend).
...
Handy API supports command-line parameters on all platforms for integration with scripts, window managers, and autostart configurations.
...
- **Opening an issue:** Read [`.github/ISSUE_TEMPLATE/`](.github/ISSUE_TEMPLATE/). Blank issues are disabled; pick the right template (`bug_report.md` for bugs). Feature requests do not belong in issues — they go to [Handy API Discussions](https://github.com/MakinaX/Handy-Api/discussions) (see `.github/ISSUE_TEMPLATE/config.yml`).
- **Proposing a feature:** Handy API is under a feature freeze. New features require community support gathered in [Handy API Discussions](https://github.com/MakinaX/Handy-Api/discussions) before any PR is opened — see the PR template's "Community Feedback" section.
