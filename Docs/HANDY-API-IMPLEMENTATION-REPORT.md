...
- The statically parsed `upstream-sync.yml` is configured to auto-resolve only
  ...
  installed runtime/DLL smoke without any signing-secret context.
- A separate `handy-api-signing` required-reviewer job downloads only that
  exact artifact and never checks out or executes candidate source. Tauri CLI
  2.11.4 is installed before secrets are exposed; only the signer step receives
  the environment-level private key/password. A later secret-free step verifies
  the generated signature with the exact candidate's committed public key and
  a digest-pinned Minisign binary.
...
5. Only after every blocking item passes, separately approve
